...
let demographics = null; // collected in Session 1 only
...
function startExp() {
  ...
  // Session 1 only: show demographic form first
  if (parseInt(SESS) === 1) {
    show("s-demo");
  } else {
    show("s-exp");
...
function submitDemo() {
  const fields = ["gender","age","level","field","ai"];
  let valid = true;
  fields.forEach(f => {
    const checked = document.querySelector(`input[name="d-${f}"]:checked`);
    const err = document.getElementById(`err-${f}`);
    if (!checked) { err.style.display = "block"; valid = false; }
    else err.style.display = "none";
  });
  if (!valid) return;

  demographics = {
    gender:       document.querySelector('input[name="d-gender"]:checked').value,
    age_group:    document.querySelector('input[name="d-age"]:checked').value,
    level:        document.querySelector('input[name="d-level"]:checked').value,
    field:        document.querySelector('input[name="d-field"]:checked').value,
    ai_frequency: document.querySelector('input[name="d-ai"]:checked').value,
  };

  logEv("demographics_submitted", JSON.stringify(demographics));
  show("s-exp");
  load(0);
}
  ...
  try {
    await fetch(SHEETS_URL, {
      ...
      body: JSON.stringify({
        ...
        language: LANG,
        demographics: demographics || null, // only present in Session 1
        result
      })
    });
    syncDot("saved");
...
function buildCSV() {
  const headers = [
    "pid","session","date","language",
    "gender","age_group","academic_level","field_of_study","ai_frequency",
    "task_num","task_id","task_type","difficulty",
  ...
  const rows = results.map(r => [
    PID, SESS, DATE, LANG,
    d.gender || "", d.age_group || "", d.level || "", d.field || "", d.ai_frequency || "",
